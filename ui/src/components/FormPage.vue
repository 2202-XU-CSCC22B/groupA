<template>
  <div class="part1">
       ...
       <q-select v-model="selectedTransaction" :options="nature" @input="handleOptionChange"></q-select>

       <div v-if="selectedTransaction.includes('sales')">
         <p>Selected Transaction: {{ selectedTransaction }}</p>
         <br>
       ...
       <div v-if="selectedTransaction.includes('borrowed')">
         ...
         <input type="text" v-model="borrowed_return_date" placeholder="Enter full address" />
         <br>
...
methods: {
  handleOptionChange : () => {
      ...
      } else if (selectedTransaction === 'transfer_loc') {
        transfer_from.value = '';
        transfer_to.value = '';
        transfer_form_number.value = '';
      } else if (selectedTransaction === 'repair_replacement') {
